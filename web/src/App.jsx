...
function Login({ onSuccess }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      if (!res.ok) {
        const t = await res.json().catch(() => ({ error: 'خطأ' }))
        throw new Error(t.error || 'فشل تسجيل الدخول')
      }
    ...
    <div style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', background: '#0f172a' }}>
      <form onSubmit={submit} style={{ background: 'rgba(17,24,39,.95)', padding: 20, borderRadius: 12, minWidth: 320, color: '#e5e7eb', border: '1px solid #1f2937' }}>
        ...
        <label>البريد الإلكتروني
          <input value={email} onChange={e => setEmail(e.target.value)} required style={{ width: '100%', padding: 10, marginTop: 6, borderRadius: 8, border: '1px solid #334155', background: '#0b1220', color: '#e5e7eb' }} />
        </label>
        <label style={{ display: 'block', marginTop: 10 }}>كلمة المرور
          <input type="password" value={password} onChange={e => setPassword(e.target.value)} required style={{ width: '100%', padding: 10, marginTop: 6, borderRadius: 8, border: '1px solid #334155', background: '#0b1220', color: '#e5e7eb' }} />
        </label>
...
export default function App() {
  ...
  useEffect(() => {
    if (!authed) return
    apiFetch('/api/invoices').then(r => r.json()).then(setInvoices).catch(() => { })
  }, [authed])

  function logout() {
    localStorage.removeItem('hf_token'); localStorage.removeItem('hf_user'); setAuthed(false)
  }
  ...
  return (
    <div style={{ padding: 16, fontFamily: 'system-ui' }}>
      ...
      <p style={{ opacity: .7 }}>الدخول محمي بتوكن (JWT) للتجربة.</p>
    </div>
  )
}